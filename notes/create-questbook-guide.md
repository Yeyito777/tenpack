...
Purpose: source text and design notes for the generated FTB Quests questbook. Keep the questbook practical: enough to explain the progression, not a full tutorial replacement for JEI/Create ponder scenes.

Status: initial FTB Quests implementation is generated by `tools/generate-ftb-questbook.py` into both `client/config/ftbquests/quests/` and `server/config/ftbquests/quests/`. The first pass uses advancement tasks for concrete Create-era milestones and checkmark tasks for manual/social guidance.
...
5. **Food Logistics**
   - Text: Farms and kitchens start early, but stocked prepared-food depots are smart-logistics work. Stock a kitchen depot, pack lunch baskets, and keep substantial prepared food ready for work crews without implying a single official ration.
   - Completion: advancement task for the Tenpack `food_logistics` milestone; player text should encourage any useful prepared-meal automation, not a required meal list.
...
4. **Ration Routes**
   - Text: Connect the kitchen to the route network. A station, stock link, package handoff, depot basket, and lunch basket turn food into faction infrastructure. The shipped food can be whatever the faction actually eats.
   - Completion: advancement task for the Tenpack `ration_routes` milestone; examples are guidance, not a canonical food ration.
...
3. **Kitchen Line**
   - Text: Belts, basins, fans, deployers, and Farmer's Delight ingredients can become a visible faction food district. Heated mixers now scale stews, soups, pasta, rice dishes, and a steak-and-potatoes dry ration line.
   - Completion: hold a prepared Farmer's Delight meal or manual/team quest.
4. **Do Not Stop at the Kitchen**
   - Text: The first kitchen teaches meals; the later logistics chapter teaches how to ship them. Food should become supply infrastructure, not a standalone tech tree.
   - Completion: informational quest.
...
1. **Physics Assembler**
   - Text: Physics Age is currently a generic vehicle-platform milestone. It follows precision/control infrastructure for readability; do not force it into early-game unless playtests show a clear reason.
...
1. **Cannon Builder / Cannon Drill**
   - Text: Artillery is a faction technology, not a starter weapon. It should require a visible industrial base with diesel-engine infrastructure behind it.
   - Completion: hold `createdieselgenerators:diesel_engine` and `createbigcannons:cannon_builder` or `createbigcannons:cannon_drill`.
2. **Cannon Mounts**
